...
+ (instancetype) createFromTakePictureArguments:(CDVInvokedUrlCommand*)command
{
    ...
    pictureOptions.overLayerMode = (OVERLAY_MODE)[[command argumentAtIndex:12 withDefault:0] integerValue];
...
//        cameraPicker.cameraViewTransform = CGAffineTransformMakeTranslation(0, (screenBounds.height - camViewHeight) / 2.0);
//        cameraPicker.cameraViewTransform = CGAffineTransformScale(cameraPicker.cameraViewTransform, scale, scale);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //add by zhouhaifeng 添加摄像头遮罩
            if (pictureOptions.overLayerMode >= OVERLAY_MODE_ID_FRONT && pictureOptions.overLayerMode <= OVERLAY_MODE_PASSPORT)
            {
                RFCameraOverlayView *overlayView = [[RFCameraOverlayView alloc] initWithFrame:cameraPicker.view.frame];
                overlayView.parentView = cameraPicker;
                [overlayView initOverlay: pictureOptions.overLayerMode];
...
- (NSData*)processImage:(UIImage*)image info:(NSDictionary*)info options:(CDVPictureOptions*)options
{
    ...
    switch (options.encodingType) {
        case EncodingTypePNG:
            data = UIImagePNGRepresentation([self imageWithImage:image scaledToSize:CGSizeMake(image.size.width * 0.5, image.size.height * 0.5)]) ;
            break;
        case EncodingTypeJPEG:
        {
            if ((options.allowsEditing == NO) && (options.targetSize.width <= 0) && (options.targetSize.height <= 0) && (options.correctOrientation == NO)){
                ...
                data = UIImageJPEGRepresentation([self imageWithImage:image scaledToSize:CGSizeMake(image.size.width * 0.5, image.size.height * 0.5)],0.1) ;
            } else {
...
- (CDVPluginResult*)resultForImage:(CDVPictureOptions*)options info:(NSDictionary*)info
{
    ...
    switch (options.destinationType) {
        ...
        case DestinationTypeDataUrl:
        {
            ...
            NSData* data = [self processImage:image info:info options:options];
            if (data)  {
                NSString *picData = [NSString stringWithFormat:@"data:image/jpeg;base64,%@",toBase64(data)];
                NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:picData, @"pic", [NSNumber numberWithInteger: (int)options.overLayerMode], @"type", nil];
...
- (void)imagePickerControllerReturnImageResult
{
    ...
    switch (options.destinationType) {
        ...
        case DestinationTypeDataUrl:
        {
            NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:toBase64(self.data), @"pic", [NSNumber numberWithInteger: (int)options.overLayerMode], @"type", nil];
